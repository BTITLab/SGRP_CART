...
saveRDS(fs1, file.path(rdsDir, "flowSet_CD45_gated.rds"))
...
saveRDS(cofactors2, file.path(rdsDir, "cofactors_add_tumor_BFPcofact3.rds"))
# cofactors2 <- readRDS(file.path(rdsDir, "cofactors_add_tumor_BFPcofact3.rds"))


fs1.VS <- transFlowVS(fs1, channels=names(cofactors2), cofactors2)
saveRDS(fs1.VS, file.path(rdsDir, "flowSet_CD45_gated_arcsinh_transformed.rds"))

# fs1.VS <- readRDS(file.path(rdsDir, "flowSet_CD45_gated_arcsinh_transformed.rds"))
...
setwd("plots/PeacoQC_results/fcs_files")
...
pdf(file.path(plotDir, "PeacoQC_results/heatmap_overview.pdf"))
...
report <- read.delim(file.path(plotDir, "PeacoQC_results/PeacoQC_report.txt"))
report <- report%>%
  left_join(x, by = "Filename")%>%
  relocate(name)


write.csv(report, file.path(plotDir, "PeacoQC_results/PeacoQC_report.csv"), row.names = F)
